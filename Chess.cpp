...
int Win() {
    ...
    if(K){
        cout<<"Player 2 (Black) wins! White King captured.\n";
        return 1;
    } 
    if(k){
        cout<<"Player 1 (White) wins! Black King captured.\n";
        return 1;
    }
    return 0;
}
...
int main() {
    ...
    cout<<"White plays first and is represented by lowercase characters\n";
    cout<<"Black plays second and is represented by uppercase characters\n\n";
    while(1){
        cout<<"Player "<<cp<<"'s turn\n Enter the starting position (row col): ";
        cin>>a>>b;
        cout<<"Enter the ending position (row col): ";
        cin>>n>>m;
        char p='\0';
        if(! (a<0 || a>7 || b<0 || b>7 || n<0 || n>7 || m<0 ||m>7))  p = board[a][b];
